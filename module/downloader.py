...
class TelegramRestrictedMediaDownloader(Bot):
    ...
    async def callback_data(self, client: pyrogram.Client, callback_query: pyrogram.types.CallbackQuery):
        ...
        if callback_data is None:
            ...
            _false_text: str = ''
            _choice: str = ''
            res: Union[bool, None, List[str]] = None
            if callback_data == BotCallbackText.LINK_TABLE:
                ...
                res: Union[bool, None, List[str]] = self.app.print_link_table(DownloadTask.TASKS)
                ...
                res: Union[bool, None, List[str]] = self.app.print_count_table()
                ...
                res: Union[bool, None, List[str]] = self.app.print_upload_table(UploadTask.TASKS)
            if res:
                # 表格可能较长,按分片发送:首片编辑原消息,其余追加新消息。
                await self.send_rich_table(message=callback_query.message, chunks=res)
        ...
        while True:
            ...
            try:
                while self.app.current_task_num < self.app.max_download_task:
                    item = DownloadTask.pick_pending()
                    if item is None:
                        break
                    ...
                    await self.__add_task(
                        ...
                        with_upload=item.get('with_upload'),
                        diy_download_type=item.get('diy_download_type')
                    )
                    dispatched = True
    ...
    async def create_download_task(
        ...
        try:
            ...
            task: DownloadTask = DownloadTask.get_or_create(link)
            task.link_type = link_type
            task.chat_id = chat_id
            task.member_num = member_num
            ChatInfo.add(chat=getattr(message, 'chat', None), chat_id=chat_id)
            DownloadTask.add_item(
                link=link,
                chat_id=chat_id,
                message=message,
                link_type=link_type,
